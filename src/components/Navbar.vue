...
const navLinks = [
  { label: "Beranda", href: "#hero", section: "hero" },
  { label: "Tentang", href: "#about", section: "about" },
  { label: "Pendidikan", href: "#education", section: "education" },
...
<template>
  <nav :class="[
        ...
        <ul class="hidden md:flex items-center gap-1 font-medium tracking-wide">
          <li v-for="link in navLinks" :key="link.section">
            <a :href="link.href" :class="[
              ...
              <span v-if="activeSection === link.section"
                class="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-emerald-500"></span>
            </a>
          </li>
        </ul>
    ...
    <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2"
      ...
      <div v-show="isOpen" class="md:hidden border-t border-emerald-50 bg-white/98 backdrop-blur-md">
        <ul class="px-4 pt-2 pb-5 space-y-1 font-medium">
          <li v-for="link in navLinks" :key="link.section">
            <a :href="link.href" @click="closeMenu" :class="[
